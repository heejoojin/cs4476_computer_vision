...
def nearest_neighbor_classify(train_image_feats,
    ...
    for i in range(sorted_idx.shape[1]):
        unique, unique_counts = np.unique(sorted_idx[:k, i], return_counts=True)
        pred_labels.append(train_labels[unique[np.argmax(unique_counts)]])
...
def kmeans(feature_vectors, k, max_iter=100):
    ...
    centroids = feature_vectors[unique_indices[:k], :]
    for _ in range(max_iter):
        min_idx = np.argmin(pairwise_distances(centroids, feature_vectors), axis=0)
        for i in range(k):
            centroids[i] = np.mean(feature_vectors[min_idx == i], axis=0)
...
def build_vocabulary(image_arrays, vocab_size=50, stride=20, max_iter=10):
    ...
    *   The default vocab_size of 50 is sufficient for you to get a
    ...
    for i in range(len(image_arrays)):
        xv, yv = generate_sample_points(image_arrays[i].shape[0], image_arrays[i].shape[1], stride)
        img = torch.Tensor(image_arrays[i]).view((1, 1, image_arrays[i].shape[0], image_arrays[i].shape[1]))
...
def kmeans_quantize(raw_data_pts, centroids):
    ...
    indices = np.zeros((raw_data_pts.shape[0],), dtype='int64')
    for i in range(0, len(D)):
        indices[i] = np.where(D[i] == np.min(D[i]))[0][0]
