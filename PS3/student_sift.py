...
def get_magnitudes_and_orientations(dx, dy):
    ...
    magnitudes = np.sqrt(dx**2 + dy**2) # indesity of the pixel
    orientations = np.arctan2(dy, dx) # direction of the pixel
...
def get_feat_vec(x,y,magnitudes, orientations,feature_width):
    ...
    pw = int(feature_width // 4)

    opatch = orientations[y-fw:y+fw, x-fw:x+fw]
    mpatch = magnitudes[y-fw:y+fw, x-fw:x+fw]
    
    for i in range(pw):
        for j in range(pw):
            _o = opatch[i*pw:i*pw+pw, j*pw:j*pw+pw]
            _m = mpatch[i*pw:i*pw+pw, j*pw:j*pw+pw]

            hist, bins = np.histogram(_o, bins=8, range=(-np.pi, np.pi), weights=_m)
            fv_idx = i*pw*fw + j*fw
            fv[fv_idx:fv_idx+8] = hist
            
    fv = (fv / (np.linalg.norm(fv) + 1e-8)) ** 0.9
