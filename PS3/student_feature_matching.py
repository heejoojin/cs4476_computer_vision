...
def match_features(features1, features2, x1, y1, x2, y2):
    ...
    sd_idx = np.argsort(dists, axis=1)

    confidences = sd[:, 0] / (sd[:, 1] + 1e-8)

    idx1 = np.argwhere(confidences <= threshold).flatten() # feature1
    idx2 = sd_idx[confidences <= threshold, 0] # feature2
