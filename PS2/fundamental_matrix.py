...
def point_line_distance(line: np.ndarray, point: np.ndarray) -> float:
    ...
    error = (a*u + b*v + c) / np.sqrt(np.square(a) + np.square(b)) # element wise
...
def signed_point_line_errors(x_0s: np.ndarray, 
    ...
    for i in range(N):
        d0 = point_line_distance(np.dot(F, x_1s[i, :]), x_0s[i, :])
        d1 = point_line_distance(np.dot(np.transpose(F), x_0s[i, :]), x_1s[i, :])
